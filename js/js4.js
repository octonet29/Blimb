$(function () {
  ...
  $(".mini").click(function () {
    if ($(this).hasClass("active")) {
      ...
      $(".img").removeClass("active").addClass("disabled");
    }
  ...
  for (var point = 1; point < 11; point++) {
    ...
    $(
      '<div class="point_volume" data-volume=" ' + pointVolume + ' "></div>'
    ).prependTo(".slide_line_volume");
  }
  ...
  function languageSide(language) {
    if (language == "ua") {
      ...
      textAlert = ["Get up", "Time is coming"];
  ...
  if (typeOfLanguage == undefined) {
    ...
    } else if (broserLanguage.indexOf("uk") != -1) {
  ...
  } else if (typeOfLanguage == "ukraine") {
    $("body").addClass("ukraine");
    setTimeout(function () {
      languageSide("ua");
    }, 100);
    $(".language_panel").find("span").text("Ua");
  } else if (typeOfLanguage == "russian") {
    $("body").addClass("russian");
    setTimeout(function () {
      languageSide("ru");
    }, 100);
  ...
  function resizeWindow() {
    ...
    if (
      ...
      $(".massiveBlock.first").hasClass("active")
    ) {
  ...
  if (
    getCookie("dates") !== undefined &&
    isInteger(parseInt(getCookie("dates")))
  ) {
    ...
    time = setInterval(function () {
      ...
      if (timeIn == 0) {
        ...
        if (howMatch < parseInt($(".massiveBlock").size()) - 1) {
          ...
          var postTimeSprint =
            day * 24 * 3600 + hour * 3600 + minutes * 60 + seconds;

          setCookie("dates", parseInt(postTimeSprint));
          $(".massiveBlock").removeClass("active");
          $(".last").addClass("active");
          setTimeout(function () {
            $(".massiveBlock.first").addClass("active");
            $(".extended_counter").addClass("active");
  ...
  function getUpBaby() {
    if (blickIcon) {
      ...
      $("head").append(
        $('<link rel="shortcut icon" type="image/x-icon"/>').attr(
          "href",
          "" + faviconsArray[faviconText] + ""
        )
      );

      if (faviconText % 2 == 0) {
        $("title").text(textAlert[0]);
      } else if (faviconText % 1 == 0) {
